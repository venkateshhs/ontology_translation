    ...
    for folder in folders:
        ...
        if os.path.exists(csv_file_path):
            ...
            if 'Total Tools' in df.columns and 'Year' in df.columns:
                ...
                subset_df = df[['Year', 'Percentage of Tools Matched']]

                # Create a pivot table
                pivot_table = pd.pivot_table(subset_df, values='Percentage of Tools Matched', index=None, columns='Year', aggfunc='sum', fill_value=0)
...
pivot_table_csv_path = os.path.join(parent_directory, f'{folder}_percentage_pivot_table_result.csv')
pivot_table_df = pivot_table_df.set_index('Subdirectory').reset_index()
