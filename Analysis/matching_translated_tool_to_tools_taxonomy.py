...
columns = ["Matched Translated Tool_Url", "Matched SX_TERM", "Matched SX_CLASS_LABEL", "SX_CLASS_ID", "SX_CLASS_NR"]
results = []

for _, row_translated in df_translated_urls.iterrows():
    translated_parent_url = row_translated["Translated Parent_Url"]
    translated_tool_url = row_translated["Translated Tool_Url"]

    translated_parent_url = translated_parent_url.lower() if isinstance(translated_parent_url, str) else ""
    translated_tool_url = translated_tool_url.lower() if isinstance(translated_tool_url, str) else ""

    matches = df_tools[
        df_tools["SX_TERM"].isin(translated_tool_url.split()) |
        df_tools["SX_CLASS_LABEL"].isin(translated_tool_url.split())
        ]
...
output_file_path = r"C:\Users\Vishwas\Desktop\Thesis\tool_fetch_dbpedia\Second Revison\To SHow it to professor Full output\matching_results.csv"
